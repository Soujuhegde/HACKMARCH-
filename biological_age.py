...
def score_bmi(bmi: float) -> float:
    ...
    elif bmi < 30:        return 1.5
    elif bmi < 35:        return 4.0
    else:                 return 7.0

def score_sleep(hours: float) -> float:
    if 7 <= hours <= 9:             return 0.0
    elif 6 <= hours < 7 or 9 < hours <= 10: return 1.5
    elif 5 <= hours < 6:            return 3.5
    else:                           return 6.0

def score_steps(steps: int) -> float:
    if steps >= 10000:              return -1.0
    elif steps >= 7500:             return 0.0
    elif steps >= 5000:             return 1.5
    elif steps >= 2500:             return 3.5
    else:                           return 6.0

def score_blood_pressure(systolic: int, diastolic: int) -> float:
    if systolic < 120 and diastolic < 80:   return 0.0
    elif systolic < 130 and diastolic < 80: return 1.0
    elif systolic < 140 or diastolic < 90:  return 3.0
    else:                                    return 6.0

def score_resting_hr(bpm: int) -> float:
    if 50 <= bpm <= 65:   return -0.5
    elif bpm <= 75:        return 0.0
    elif bpm <= 85:        return 1.5
    elif bpm <= 100:       return 3.0
    else:                  return 5.0

def score_smoking(smoker: bool, packs_per_day: float = 0) -> float:
    if not smoker:                  return 0.0
    elif packs_per_day <= 0.5:     return 3.0
    elif packs_per_day <= 1:       return 5.0
    else:                           return 8.0

def score_stress(level: int) -> float:
    if level <= 3:   return 0.0
    elif level <= 5: return 1.0
    elif level <= 7: return 2.5
    else:            return 4.5

def score_exercise(minutes_per_week: int) -> float:
    if minutes_per_week >= 300:    return -1.5
    elif minutes_per_week >= 150:  return 0.0
    elif minutes_per_week >= 75:   return 1.5
    elif minutes_per_week >= 30:   return 3.0
    else:                           return 5.0

def score_diet(quality: int) -> float:
    if quality >= 8:   return -0.5
    elif quality >= 6: return 0.0
    elif quality >= 4: return 1.5
    else:              return 3.5

def score_alcohol(units_per_week: int) -> float:
    if units_per_week == 0:     return 0.0
    elif units_per_week <= 7:   return 0.5
    elif units_per_week <= 14:  return 2.0
    else:                        return 4.0
...
def calculate_biological_age(metrics: dict) -> dict:
    ...
    breakdown = {
        k: {"score": round(v[0], 2), "weight": v[1], "contribution": round(v[0] * v[1], 2)}
